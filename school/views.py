...
#for showing signup/login button for student(by sumit)
def panchaythsecretaryclick_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return render(request,'school/panchayathsecretaryclick.html')
def phcclick_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return render(request,'school/phcclick.html')
...
def afterlogin_view(request):
    ...
    elif is_phc(request.user):
        return redirect('phc-dashboard')
    elif is_panchayathsecretary(request.user):
        return redirect('panchayathsecretary-dashboard')
...
def phc_view_teacher_view(request):
    teachers=models.TeacherExtra.objects.all().filter(status=True)
    return render(request,'school/phc_view_teacher.html',{'teachers':teachers})

@login_required(login_url='panchayathsecretarylogin')
@user_passes_test(is_panchayathsecretary)
def panchayathsecretary_view_teacher_view(request):
    teachers=models.TeacherExtra.objects.all().filter(status=True)
...
def teacher_student_view(request):
    return render(request,'school/teacher_student.html')
def phc_student_view(request):
...
def phc_view_student_view(request):
    students=models.StudentExtra.objects.all().filter(status=True)
    return render(request,'school/phc_view_student.html',{'students':students})

@login_required(login_url='panchayathsecretarylogin')
@user_passes_test(is_panchayathsecretary)
def panchayathsecretary_view_student_view(request):
    students=models.StudentExtra.objects.all().filter(status=True)
    return render(request,'school/panchayathsecretary_view_student.html',{'students':students})
...
@login_required(login_url='teacherlogin')
#@user_passes_test(is_admin)
def teacher_approve_student_view(request):
    students=models.StudentExtra.objects.all().filter(status=False)
    return render(request,'school/teacher_approve_student.html',{'students':students})
...
@login_required(login_url='phclogin')
@user_passes_test(is_phc)
def phc_dashboard_view(request):
    teacherdata=models.TeacherExtra.objects.all().filter(status=True,user_id=request.user.id)
    notice=models.Notice.objects.all()
    # mydict={
    #     'salary':teacherdata[1].salary,
    #     'mobile':teacherdata[1].mobile,
    #     'date':teacherdata[1].joindate,
    #     'notice':notice
    # }
    return render(request,'school/phc_dashboard.html')

@login_required(login_url='panchayathsecretarylogin')
@user_passes_test(is_panchayathsecretary)
def panchayathsecretary_dashboard_view(request):
    teacherdata=models.TeacherExtra.objects.all().filter(status=True,user_id=request.user.id)
    notice=models.Notice.objects.all()
    # mydict={
    #     'salary':teacherdata[1].salary,
    ...
    #     'notice':notice
    # }
    return render(request,'school/panchayathsecretary_dashboard.html')
